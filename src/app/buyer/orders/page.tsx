      ...
      <main className="min-h-screen bg-[#F5F9F9] text-zinc-900 pb-20">
        ...
        <div className="container mx-auto px-6 md:px-12 pt-44">
          <div className="py-20 bg-white rounded-[40px] flex flex-col items-center justify-center text-zinc-400 border-2 border-dashed border-zinc-100">
            ...
            <p className="font-bold">{dict["orders.pleaseLogin"]}</p>
          </div>
        </div>
      </main>
    );
    ...
    <main className="min-h-screen bg-[#F5F9F9] text-zinc-900 pb-20">
      ...
      <div className="container mx-auto px-6 md:px-12 pt-44">
        <div className="flex items-center gap-4 mb-10">
           ...
           <div>
              <h1 className="text-4xl font-black tracking-tighter">{dict["orders.title"]}</h1>
              <p className="text-zinc-500 font-medium">{dict["orders.subtitle"]}</p>
           </div>
        </div>
        ...
        <div className="space-y-8">
          {serializedOrders.length === 0 ? (
            <div className="py-20 bg-white rounded-[40px] flex flex-col items-center justify-center text-zinc-400 border-2 border-dashed border-zinc-100">
              <ShoppingBag className="w-16 h-16 mb-4 opacity-10" />
              <p className="font-bold">{dict["orders.empty"]}</p>
            </div>
          ) : (
              ...
              <div key={order.id} className="bg-white rounded-[40px] overflow-hidden border border-zinc-100 shadow-sm">
                <div className="px-8 py-5 bg-zinc-50/50 flex flex-wrap justify-between items-center border-b border-zinc-100 gap-4">
                  <div className="flex items-center gap-6">
                    <div>
                        <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest leading-none mb-1">{dict["orders.masterOrderId"]}</p>
                        <p className="text-sm font-bold text-zinc-900">#{order.id.slice(0, 8)}</p>
                    </div>
                    <div className="h-8 w-px bg-zinc-200" />
                    <div>
                        <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest leading-none mb-1">{dict["orders.purchaseDate"]}</p>
                        <p className="text-sm font-bold text-zinc-600">{new Date(order.createdAt).toLocaleDateString(lang === 'vi' ? 'vi-VN' : 'en-US')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest border ${
                        ...
                        {order.paymentStatus === 'PAID' ? dict["orders.paid"] : dict["orders.pendingPayment"]}
                    </span>
                  </div>
                </div>
                    ...
                    <div key={subOrder.id} className="p-8">
                      <div className="flex flex-wrap justify-between items-start mb-8 gap-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest leading-none mb-1">{dict["orders.shipping"]}</p>
                            <p className="text-sm font-bold">{subOrder.trackingCode || dict["orders.waitingSellerShip"]}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                           ...
                           <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                            ...
                            {subOrder.status === 'PENDING' ? dict["orders.pending"] :
                             subOrder.status === 'SHIPPING' ? dict["orders.shippingStatus"] :
                             subOrder.status === 'DELIVERED' ? dict["orders.delivered"] :
                             subOrder.status === 'COMPLETED' ? dict["orders.completed"] :
                             subOrder.status === 'DISPUTED' ? dict["orders.dispute"] :
                             subOrder.status === 'REFUNDED' ? dict["orders.refunded"] : subOrder.status}
                          </span>
                        </div>
                      </div>
                          ...
                          <div key={item.id} className="flex gap-6 items-center">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-zinc-500 font-medium">
                                {dict["orders.quantity"]?.replace("{count}", String(item.quantity))}
                              </p>
                            </div>
                            <div className="text-right">
                               <p className="text-zinc-400 text-[10px] font-black uppercase">{dict["orders.unitPrice"]}</p>
                               <p className="font-bold text-zinc-900">{new Intl.NumberFormat(lang === 'vi' ? 'vi-VN' : 'en-US', { style: 'currency', currency: 'VND' }).format(Number(item.priceAtPurchase))}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                <div className="bg-zinc-50/50 px-8 py-6 flex flex-col items-end gap-2 border-t border-zinc-100">
                  <div className="flex justify-between w-full max-w-xs text-sm font-medium">
                    <span className="text-zinc-400 uppercase tracking-widest text-[10px] font-black">{dict["orders.subtotal"]}</span>
                    <span className="text-zinc-900">
                      {new Intl.NumberFormat(lang === 'vi' ? 'vi-VN' : 'en-US', { style: 'currency', currency: 'VND' }).format(order.subOrders.reduce((acc: number, so: any) => acc + Number(so.subTotal), 0))}
                    </span>
                  </div>
                  {Number(order.discountAmount) > 0 && (
                    <div className="flex justify-between w-full max-w-xs text-sm font-medium">
                      <span className="text-zinc-400 uppercase tracking-widest text-[10px] font-black">{dict["orders.voucherDiscount"]}</span>
                      <span className="text-red-500">
                        -{new Intl.NumberFormat(lang === 'vi' ? 'vi-VN' : 'en-US', { style: 'currency', currency: 'VND' }).format(Number(order.discountAmount))}
                      </span>
                    </div>
                  )}
                  <div className="flex justify-between w-full max-w-xs pt-4 mt-2 border-t border-zinc-200 items-center">
                    <span className="text-xs font-black uppercase tracking-widest text-zinc-400">{dict["orders.totalPayment"]}</span>
