...
export function OrderActionButtons({ subOrder, discountAmount = 0, grandSubTotal = 0 }: OrderActionButtonsProps) {
  ...
  // Dispute form state
  const [disputeReason, setDisputeReason] = useState(t("orders.disputeReason1"));
  ...
  const handleComplete = async () => {
    if (!confirm(t("orders.completeConfirm"))) return;
    setLoading(true);
  ...
  const handleCancel = async () => {
    ...
    if (res.success) {
      ...
      alert(res.error || t("orders.cancelError"));
    }
    setLoading(false);
  };
  ...
  return (
    <div className="flex flex-wrap gap-3 pt-6 border-t border-zinc-100">
      <div className="flex-1">
        <span className="text-sm text-zinc-500 font-medium">{t("orders.paymentLabel")}</span>
        <div className="text-xl font-black text-primary">
          {new Intl.NumberFormat(language === 'vi' ? 'vi-VN' : 'en-US', { style: 'currency', currency: 'VND' }).format(finalDisplayTotal)}
        </div>
        {subOrderDiscount > 0 && (
      ...
      <div className="flex gap-2">
          ...
          <>
            {subOrder.status === "PENDING" ? (
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-zinc-50 text-zinc-400 font-bold rounded-lg border border-zinc-100 text-sm italic">
                {subOrder.status === "CONFIRMED" ? t("orders.sellerConfirmed") : t("orders.sellerPacking")}
              </div>
            )}
          </>
        )}
        ...
        {subOrder.status === "COMPLETED" && subOrder.review && (
           <span className="text-xs font-bold text-green-600 flex items-center gap-1">
             <CheckCircle2 className="w-4 h-4" /> {t("orders.reviewed")}
           </span>
        )}
      </div>
            ...
            >
               <div className="bg-[#f4511e] p-6 text-white flex justify-between items-center">
                  <h3 className="text-lg font-black flex items-center gap-2">
                    <AlertCircle className="w-5 h-5" /> {t("orders.disputeTitle")}
                  </h3>
                  <button onClick={() => setShowDisputeModal(false)}><X /></button>
               </div>
               <form onSubmit={handleDispute} className="p-8 space-y-6">
                  <div>
                    <label className="text-[10px] font-black uppercase text-zinc-400 mb-2 block">{t("orders.disputeReason")}</label>
                    <select 
                      value={disputeReason} 
                      onChange={(e) => setDisputeReason(e.target.value)}
                      className="w-full p-4 bg-zinc-50 border border-zinc-100 rounded-xl outline-none focus:ring-2 focus:ring-[#f4511e]/20"
                    >
                      <option>{t("orders.disputeReason1")}</option>
                      <option>{t("orders.disputeReason2")}</option>
                      <option>{t("orders.disputeReason3")}</option>
                      <option>{t("orders.disputeReason4")}</option>
                      <option>{t("orders.disputeReason5")}</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase text-zinc-400 mb-2 block">{t("orders.disputeDesc")}</label>
                    <textarea 
                      required
                  ...
                  <div>
                    <label className="text-[10px] font-black uppercase text-zinc-400 mb-2 block">{t("orders.disputeEvidence")}</label>
                    <input 
                      type="text"
                      placeholder={t("orders.disputeEvidencePlaceholder")}
                      value={disputeImages}
                  ...
                  <button 
                    ...
                    disabled={loading}
                  >
                    {loading ? t("orders.sending") : t("orders.sendDispute")}
                  </button>
               </form>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
               <div className="bg-[#ff8200] p-6 text-white flex justify-between items-center">
                  <h3 className="text-lg font-black flex items-center gap-2">
                    <Star className="w-5 h-5" /> {t("orders.reviewTitle")}
                  </h3>
               ...
               <form onSubmit={handleReview} className="p-8 space-y-6">
                  ...
                  <div>
                    <label className="text-[10px] font-black uppercase text-zinc-400 mb-2 block">{t("orders.yourComment")}</label>
                    <textarea 
                      required
                  ...
                  <button 
                    ...
                    disabled={loading}
                  >
                    {loading ? t("orders.sending") : t("orders.sendReview")}
                  </button>
               </form>
            </motion.div>
          </div>
